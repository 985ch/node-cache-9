...
class RedisCache extends BaseCache {
  constructor(config, errLog) {
    ...
    if (!this.rds) throw new Cache9Error('invaild redis client');
  }
  // 获取缓存内容
  ...
  setCache(key, data, options = {}) {
    ...
    const raw = this._getCfg(options, 'raw', false);
    if (ttl === 0) {
      this.rds.set(this._getKey(key), raw ? data : JSON.stringify(data));
    } else {
  ...
  setCacheM(key, subkey, data = null, options = {}) {
    ...
    const raw = this._getCfg(options, 'raw', false);
    if (ttl === 0) {
      this.rds.set(this._getMKey(key, subkey), raw ? data : JSON.stringify(data));
    } else {
  ...
  async _getCacheM(key, keys, options) {
    ...
    if (raw && cacheUndefined) throw new Cache9Error('raw must be false when cacheUndefined is true');
